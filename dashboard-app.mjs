...
function sectorRow(def,s,old){
  ...
  return `<article class="sector-row" id="sector-${def.id}" aria-labelledby="name-${def.id}"><div class="sector-main">
    ...
    <div class="sector-note"><dl class="sector-analysis"><div><dt>结构</dt><dd>${esc(insight.structure)}</dd></div><div><dt>变化</dt><dd>${esc(insight.delta)}</dd></div><div><dt>观察</dt><dd>${esc(insight.condition)}</dd></div></dl>${news?`<details class="sector-news"><summary>${esc(newsLabel)} · ${esc(news.publishedAt.slice(5,10))} ＋</summary><p>${esc(news.text)}</p><p class="secondary">${news.afterSnapshot?'快照后消息，价格反应待确认。':esc(news.priceRelation||'事件与当轮涨跌的因果尚未证实。')}</p><a class="note-source" href="${safeUrl(news.sources[0].url)}" target="_blank" rel="noopener noreferrer">${esc(news.sources[0].name)} ↗</a></details>`:'<p class="secondary">消息：暂无有效的新催化；不以新闻代替价格确认。</p>'}</div>
