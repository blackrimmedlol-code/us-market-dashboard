import {SECTORS,LABELS,summarize,compare,validQuote,newsFor} from './dashboard-model.mjs';
...
function sectorRow(def,s,old){
  ...
  return `<article class="sector-row" id="sector-${def.id}" aria-labelledby="name-${def.id}"><div class="sector-main">
    ...
    <div class="sector-note"><div class="note-label"><span>${esc(newsLabel)}</span>${news?`<span>${esc(news.publishedAt.slice(5,10).replace('-','/'))}</span>`:''}</div><p>${esc(news?.text||txt)}</p>${news?`<p class="secondary">${esc(txt)} ${news.afterSnapshot?'快照后消息，价格反应待确认。':esc(news.priceRelation||'事件与当轮涨跌不构成已证实的因果关系。')}</p><a class="note-source" href="${safeUrl(news.sources[0].url)}" target="_blank" rel="noopener noreferrer">${esc(news.sources[0].name)} ↗</a>`:'<p class="secondary">暂无已核实、可解释本轮变化的新催化。</p>'}</div>
...
function render(){
  ...
  const delta=compare(market.state,previous?.market.state),summary=summaries[market.state];
  ...
  <section class="hero" aria-labelledby="market-heading"><div class="eyebrow"><span id="market-heading">市场状态</span><span class="mono">MARKET REGIME</span></div><div class="hero-top"><div class="regime"><h1 class="tone-${stateTone(market.state)}">${LABELS[market.state]}</h1><span class="badge tone-${stateTone(market.state)}">${delta==='首次记录'?'首次记录':delta+' · 较上次'}</span></div><div class="hero-summary"><h2>${summary[0]}</h2><p>${summary[1]}<br>判断置信度：${market.confidence} · 描述当前结构，不预测收益。</p></div></div>
  <div class="evidence-grid"><div class="evidence"><div class="evidence-label"><span>01 / 价格结构</span><span>30 MIN</span></div><div class="evidence-value tone-${stateTone(market.price)}">${priceText}<small>QQQ / SPY</small></div><p>完整 30 分钟趋势＋日内近似均价交叉确认</p></div>
  <div class="evidence"><div class="evidence-label"><span>02 / 上涨参与度</span><span>BREADTH</span></div><div class="evidence-value ${breadthGood?'tone-'+stateTone(market.breadth):'tone-muted'}">${breadthGood?b.upPct.toFixed(1)+'%':'待确认'}<small>上涨占比 · 不含平盘</small></div><p>${breadthGood?`${b.advancing.toLocaleString()} 家上涨 / ${b.declining.toLocaleString()} 家下跌`:'未取得同一时段的可核实广度'}</p>${breadthGood?`<div class="breadth-track" aria-hidden="true"><i style="width:${b.upPct}%"></i><b style="width:${100-b.upPct}%"></b></div>`:''}</div>
  <div class="evidence"><div class="evidence-label"><span>03 / 波动压力</span><span>VIX</span></div><div class="evidence-value tone-${volGood?(a.VIX.changePct>0?'down':'up'):'muted'}">${volGood?a.VIX.price.toFixed(2):'—'}<small>${volGood?pct(a.VIX.changePct):'待确认'}</small></div><p>${volGood?(a.VIX.changePct>0?'波动预期上升':'波动预期缓和'):'缺少同一时点数据'} · 与价格和广度一起判断</p></div></div>
  <div class="benchmark-strip">${['QQQ','SPY'].map(s=>`<div class="benchmark"><b>${s}</b><span class="tone-${tone(a[s].changePct)}">${pct(a[s].changePct)}</span></div>`).join('')}<span class="benchmark-note">${phase} · 所有涨跌使用同一比较窗口</span></div></section>
